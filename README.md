...
Класс Catalog
Нужен для того-то что бы хранить и получать каталог товаров хранящихся на сервере

Поля класса:
products - массив продуктов
preview - проверяет есть или нету проекта внутри массива

Методы класса:
setProducts - Записывает данные о продуктах
getItems - Получить массив обьектов
getItemid - Получить обьект по id
setPreview - добавить обьект в массив
getPreview - получить конкретный обьект из массива
setItems - добавить несколько обьектов в массив 
...

Класс Basket
Нужен для добавления товара в корзину, пользователем
...
add - добавление товара, в массив корзины;
remove - удаление товара, из массива корзины;
clear - очистка корзины;
getTotal - получение стоимости всех товаров в корзине;
getCount - получение количества товаров в корзине;
hasProduct - проверка наличия товара в корзине по его id;
...

Класс Buyer
Нужен для добавления и получения данных пользователя
...
Методы класса:
set - сохранение данных покупателя
get - получить данные покупателя
validate - возвращает объект с ошибками
clear - очистка данных покупателя

### «Слой коммуникации»

Класс ApiShop
Нужен для разделения логики приложения и сетевого взаимодействие

Поля класса:
api - содержит экземпляр HTTP-клиента, реализующий IApi интерфейс
...
Презентер связывает модели и представления приложения, управляет состоянием и реагирует на события.
Все обработчики событий находятся в презентере (main.ts). Презентер получает данные от моделей и передаёт их в представления.